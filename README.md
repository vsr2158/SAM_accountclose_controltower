...
```bash
function_accountclose$ sam build --use-container
```
...
```bash
function_accountclose$ sam logs -n StockCheckerFunction --stack-name function_accountclose --tail
```
...
```bash
function_accountclose$ pip install -r tests/requirements.txt --user
# unit test
function_accountclose$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
function_accountclose$ AWS_SAM_STACK_NAME=<stack-name> python -m pytest tests/integration -v
```
...
```bash
aws cloudformation delete-stack --stack-name function_accountclose
```
